...
Logs to:
    ...
    data/verification_images/site_A/tag<ID>_<timestamp>.jpg       — crop around the tag
    data/verification_images/site_A/tag<ID>_<timestamp>_full.jpg  — full uncropped frame
...
def save_tag_images(frame, corners, tag_id, site):
    """Saves both the full uncropped frame and a tight crop around the tag,
    sharing the same tag<ID>_<timestamp> stem so they can be matched to the
    same CSV row."""
    verify_dir = os.path.join(DATA_DIR, "verification_images", f"site_{site}")
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    stem = f"tag{tag_id:04d}_{ts}"

    try:
        full_path = os.path.join(verify_dir, f"{stem}_full.jpg")
        cv2.imwrite(full_path, cv2.cvtColor(frame, cv2.COLOR_RGB2BGR))
    except Exception as e:
        print(f"  [WARN] Could not save full image: {e}")
    ...
    try:
        ...
        crop_path = os.path.join(verify_dir, f"{stem}.jpg")
        cv2.imwrite(crop_path, cv2.cvtColor(crop, cv2.COLOR_RGB2BGR))
    except Exception as e:
        print(f"  [WARN] Could not save crop image: {e}")
        ...
        while True:
            ...
            if ids is not None:
                for i, tag_id in enumerate(ids.flatten()):
                    ...
                    save_tag_images(frame, tag_corners, tag_id, site)
